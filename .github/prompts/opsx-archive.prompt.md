---
description: 실험적 워크플로에서 완료된 변경 아카이브
---

실험적 워크플로에서 완료된 변경을 아카이브합니다.

**입력**: `/opsx:archive` 뒤에 변경 이름을 선택적으로 지정할 수 있습니다(예: `/opsx:archive add-auth`). 생략하면 대화 문맥에서 추론 가능한지 확인합니다. 모호하거나 애매하면 반드시 사용 가능한 변경을 물어봐야 합니다.
...
2. **아티팩트 완료 상태를 확인합니다**
   ...
   - `artifacts`: 상태(`done` 또는 기타)를 가진 아티팩트 목록
...
**스키마:** <schema-name>
**아카이브 위치:** openspec/changes/archive/YYYY-MM-DD-<name>/
**Specs:** 동기화 건너뜀(사용자 선택)

**경고:**
- 미완료 아티팩트 2개 상태로 아카이브됨
- 미완료 작업 3개 상태로 아카이브됨
- 델타 스펙 동기화를 건너뜀(사용자 선택)

의도한 결과가 아니라면 아카이브를 검토하세요.
```
...
**변경:** <change-name>
**대상 경로:** openspec/changes/archive/YYYY-MM-DD-<name>/

대상 아카이브 디렉터리가 이미 존재합니다.
...
- 완료 여부 확인에는 아티팩트 그래프(`openspec status --json`)를 사용합니다
- 경고가 있어도 아카이브를 막지 말고, 안내 후 확인받아 진행합니다
- 아카이브 이동 시 `.openspec.yaml`이 보존되도록 합니다(디렉터리와 함께 이동)
- 수행 결과를 명확히 요약합니다
- 동기화가 요청되면 Skill 도구로 `openspec-sync-specs`를 호출합니다(에이전트 기반)
- 델타 스펙이 있으면 항상 동기화 평가를 수행하고 통합 요약 후 질문합니다
- 경고/요약/안내는 의미를 유지하는 최소 토큰으로 간결하게 제공합니다
